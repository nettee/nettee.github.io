...
function canvasApp() {
	...
	var snake = (function() {
		var head = {x : X * 0.4, y : Y * 0.4};
		var body = new Array();
		for (var i = 0; i < 4; i++) {
			body[i] = {x : head.x + i + 1, y : head.y}; 
		}
		return {
			head : head,
			body : body,
	...
	function generateApple() {
		...
		while (true) {
			...
			if (isOnSnake(p) || (!!apple && p.x == apple.x && p.y == apple.y)) {
				console.log('collision (' + p.x + ', ' + p.y + '), body ' + snake.body);
				continue;
			} else {
				var apple = {x : p.x, y : p.y};
				console.log('generate apple at ' + pointString(apple));
				return apple;
			}
		}
	}
